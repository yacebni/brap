document.addEventListener('DOMContentLoaded', function () {
    ...
    searchInput.addEventListener('input', function (e) {
        ...
        students.forEach(function (student) {
            ...
            const typeAdm = student.querySelector('td:nth-child(4)').textContent.toLowerCase();

            const isVisible = nomStudent.includes(value) || prenomStudent.includes(value) || classStudent.includes(value) || typeAdm.includes(value);
    ...
    studentsRows.forEach(studentRow => {
        studentRow.addEventListener('click', () => {
            const id_administratif = studentRow.dataset.idAdministratif; // Ajoutez l'attribut data-id-administratif dans votre balise tr
            if (id_administratif) {
                window.location.href = 'gerer_administratif.php?id=' + id_administratif;
